...
public class PaymentService {
    ...
    public PaymentIntentResponse createPaymentIntent(Long orderId) throws StripeException {
        ...
        if (!order.getEmail().equals(email))
            throw new APIException("You are not allowed to pay this order");
        ...
        if (payment.getStatus() == PaymentStatus.SUCCEEDED)
            throw new APIException("Payment is already completed");
        ...
        if (payment.getPaymentIntentId() != null) {
            ...
            return new PaymentIntentResponse(order.getId(), paymentIntent.getClientSecret(),
        ...
        PaymentIntentCreateParams params = PaymentIntentCreateParams.builder()
                .setAmount(amount).setCurrency(payment.getCurrency()).putMetadata("orderId", order.getId().toString())
                .build();
    ...
    private void handleSuccessfulPayment(Event event) {
        ...
        if (payment == null)
            throw new ResourceNotFoundException("Payment", "PaymentIntentId", paymentIntent.getId());
        ...
        // Look up user by order email (no auth context in webhook)
